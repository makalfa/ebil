...
function lookup_mylinkedsmshandle_id(sms_billhandlesmshandle_id) {
    if(sms_billhandlesmshandle_id.length == 0) {
        
		
		$('#sms_billhandlesmshandle_idsuggestions').hide();
    } else {
        $.post("../sms/rpcsms_billhandle.php", {queryString: ""+sms_billhandlesmshandle_id+""}, function(data){
            if(data.length >0) {
                $('#sms_billhandlesmshandle_idsuggestions').show();
                $('#sms_billhandlesmshandle_idautoSuggestionsList').html(data);
            }
        });
    }
} // sms_billhandle   lookup




function fill_sms_billhandlesmshandle_id(thisValue) {
	
    $('#sms_billhandlesmshandle_id').val(thisValue);
	
	
   $('#sms_billhandlesmshandle_idsuggestions').hide();
}

function fill_hiddensms_billhandlesmshandle_id(thisValue) {
	
    $('#sms_billhandlesmshandle_idhidden').val(thisValue);
