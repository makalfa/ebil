...
$sql="select  DATE_add(bill_date, INTERVAL $days  DAY) as 'pay_before' from sms_disconnschedule where ucase(disconnschedule_name)='$schedule'";
 ...
 if($strComtypes[0]=='bill'||$strComtypes[0]=='3bill')
...
if(($startcommtype[0]=='bill')||($startcommtype[0]=='3bill')||($startcommtype[0]=='disconnection')){

    if(($numberLen==10)&&(is_numeric($phone_number))){
     $connectionExists=checkDuplicateConnections(trim($connection_number));
    if(!$connectionExists){
    insertWaterBillHandler($phone_number,$connection_number,$amount,$pay_before,$commtype);
    }

    }else{
      splitDelimitedWaterBillNumbers($phone_number,$connection_number,$amount,$pay_before,$commtype);
    }
}
...
function uplodeGeneralsmsFileData($fileToUploade,$program,$schedule,$scheduleId){
...
$phone_number=padPhoneNumberWithZero($phone_number);
    ...
    $date_created=date('Y-m-d');
                ...
                $messagecut=str_replace('{name}',$recepient,$custmsg);
          		$message=str_replace('{message}',$additionalDetails,$messagecut);

              if(($numberLen==10)&&(is_numeric($phone_number))){

	          insertOtherProcessingInfo($recepient,$phone_number,$message,$scheduleId);
              }else{
                  splitDelimitedNumbers($recepient,$phone_number,$message,$scheduleId);
              }
			//
			}
...
function splitDelimitedWaterBillNumbers($phone_number,$connection_number,$amount,$pay_before,$sys_track){
       ...
       if(sizeof($actualfieldsArr)<2){
		...
		for($i=0;$i<sizeof($actualfieldsArr);$i++){
            ...
            if((strlen($Number)==10)&&(is_numeric($Number))){
             insertWaterBillHandler($Number,$connection_number,$amount,$pay_before,$sys_track);
            }else{
             insertInvalidAddressWaterBillHandler($Number,$connection_number,$amount,$pay_before,$sys_track);
            }
		}
}
...
function insertWaterBillHandler($phone_number,$connection_number,$amount,$pay_before,$sys_track){
        $created_by=$_SESSION['my_useridloggened'];
    $date_created=date('Y-m-d');
  $uuid=gen_uuid();
  $voided=0;

    $insertSQl= "Insert into sms_billhandle (connection_number,amount,phone_number,pay_before,date_created,created_by,voided,sys_track,uuid) values
                        ('$connection_number','$amount','$phone_number','$pay_before','$date_created','$created_by','$voided','$sys_track','$uuid')";
...
function insertInvalidAddressWaterBillHandler($phone_number,$connection_number,$amount,$pay_before,$sys_track){
    $created_by=$_SESSION['my_useridloggened'];
    $date_created=date('Y-m-d');
    $uuid=gen_uuid();
    $voided=0;
    $insertInvalidSQl= "Insert into sms_smsinvalid (connection_number,amount,phone_number,pay_before,date_created,created_by,voided,sys_track,uuid)
    values ('$connection_number','$amount','$phone_number','$pay_before','$date_created','$created_by','$voided','$sys_track','$uuid')";
    $Result1 = mysql_query($insertInvalidSQl) or die(mysql_error());
}
